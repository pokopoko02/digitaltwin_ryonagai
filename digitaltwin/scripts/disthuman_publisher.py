...
class humandist():
    def __init__(self):
        self.input_depth_n = rospy.get_param("/human_dist/input_depth_n", 18)
        self.visual_odom = rospy.get_param("/human_dist/visual_odom", False)
        self.human_detection = rospy.get_param("/human_dist/human_detection", False)
        self.camera_info = rospy.get_param("/human_dist/yolo_camera_info", "/camera/color/camera_info")
        self.max_goal_dist = rospy.get_param("/human_dist/max_goal_dist", 20)
        self.human_data = [1.0]*(self.input_depth_n)
                    ...
                    for i in range(self.input_depth_n):
                        if i == 0:
                            if box_xmin < self.img_width-self.img_width/(self.input_depth_n-1)*i-1 < box_xmax:
                                self.human_data[i] = self.input_data[i]
                        else:
                            if box_xmin < self.img_width-self.img_width/(self.input_depth_n-1)*i < box_xmax:
                                self.human_data[i] = self.input_data[i]
            #f.write(human_data,"\n")
            self.input_data[self.input_depth_n:self.input_depth_n*2] = self.human_data
    ...
    def human_publish(self):	#
        rospy.init_node('dist_human', anonymous=True)#ROS起動のノード使う時お決まり
        ...
        publisher = rospy.Publisher('/dist_h', Float32MultiArray, queue_size=10)
        r = rospy.Rate(10) # 10hz
        while not rospy.is_shutdown():
            array_for_Publish = Float32MultiArray(data = self.human_data)
            publisher.publish(array_for_Publish)#これでどうなんだろ
            print(array_for_Publish)
            r.sleep()
