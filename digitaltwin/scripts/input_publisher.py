...
class input_publisher():
    ...
    def input_publish(self):	#
        ...
        publisher2 =rospy.Publisher('/human_publish',PoseStamped,queue_size=10)
        r = rospy.Rate(10) # 10hz
        while not rospy.is_shutdown():
            #print("input_publish")
            print(self.human_data)
            array_for_Publish = Float32MultiArray(data = self.input_data)
            #array_for_Publish2 = Float32MultiArray(data = self.human_data)
            pose=PoseStamped()
            pose.pose.position.x=self.human_data[0]
            pose.pose.position.y=self.human_data[4]
            pose.pose.position.z=self.human_data[5]

            publisher.publish(array_for_Publish)
            
            #publisher2.publish(array_for_Publish2)#これでどうなんだろ
            publisher2.publish(pose)#これでどうなんだろ
            print(pose)
            #publisher2.publish()
            r.sleep()
